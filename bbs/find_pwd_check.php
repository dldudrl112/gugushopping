...
$email = trim($_POST['id_email']);

if(!$email)
    alert_close('메일주소 오류입니다.');

$sql = " select count(*) as cnt from shop_member where id = '$email' ";
$row = sql_fetch($sql);
if($row['cnt'] > 1)
    alert('동일한 메일주소가 2개 이상 존재합니다.\\n\\n관리자에게 문의하여 주십시오.');

$sql = " select index_no, id, name, email from shop_member where id = '$email' ";
$mb = sql_fetch($sql);
if(!$mb['id'])
    alert('존재하지 않는 회원입니다.');
...
// 인증 링크 생성
$href = TB_BBS_URL.'/password_lost_certify.php?mb_no='.$mb['index_no'].'&mb_nonce='.$mb_nonce;
...
$content .= '<a href="'.TB_URL.'" target="_blank">'.$config['company_name'].'</a>';
$content .= '</span>';
$content .= '<p style="margin:20px 0 0;padding:30px 30px 30px;border-bottom:1px solid #eee;line-height:1.7em">';
$content .= addslashes($mb['name'])." 회원님은 ".TB_TIME_YMDHIS." 에 회원정보 찾기 요청을 하셨습니다.<br>";
...
mailer($config['company_name'], $super['email'], $email, $subject, $content, 1);


include_once(TB_THEME_PATH.'/find_pwd_check.skin.php');
