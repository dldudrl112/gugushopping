...
if ($gs['is_adult'] && !isAdultUser($member['checked_birthday'])){
    alert("성인 인증 후 확인이 가능합니다.");
}
if(TB_IS_MOBILE) {
...
$rows = 40;
$total_page = ceil($item_use_count / $rows); // 전체 페이지 계산
if($page == "") { $page = 1; } // 페이지가 없으면 첫 페이지 (1 페이지)
$from_record = ($page - 1) * $rows; // 시작 열을 구함

$review_sql		=	" select * from shop_goods_review where gs_id = '$index_no' order by reg_time desc limit $from_record, $rows ";
...
// 상품 가격
$it_price	=	get_price($index_no);

$supply_item_gugu	=	get_item_supply_gugu($index_no, $gs['spl_subject']);
...
if($member["id"]){
	...
	$sql = "select count(wi_id) as cnt from shop_wish where gs_id='$index_no' and mb_id = '".$member["id"]."'";
	$wi = sql_fetch($sql);
...
// 대표 카테고리
$sql = "select * from shop_goods_cate a, shop_cate b
		  where a.gcate=b.catecode and  a.gs_id='$index_no' order by a.index_no asc limit 1 ";
$ca = sql_fetch($sql);
