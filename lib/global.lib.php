...
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="")
{
	...
	/*
	<div class="pager-box">
		<a href="javascript:void(0)" class="prev"><img src="<?php echo TB_IMG_URL; ?>/m-pager-prev.png" alt="prev"><img src="<?php echo TB_IMG_URL; ?>/m-pager-prev-on.png" alt="prev-on" class="on"></a>
		<div class="pager">
			<span class="now">1</span>
			/
			<span class="total">10</span>
		</div>
		<a href="javascript:void(0)" class="next on"><img src="<?php echo TB_IMG_URL; ?>/m-pager-next.png" alt="next"><img src="<?php echo TB_IMG_URL; ?>/m-pager-next-on.png" alt="next-on" class="on"></a>
	</div>
	*/
	...
	*/
	...
	if($cur_page > 1) {
		$str .= '<a href="'.$url.($cur_page-1).$add.'" class="prev"><img src="'.TB_IMG_URL.'/m-pager-prev-on.png" alt="prev-on"></a>'.PHP_EOL;
	} else {
		$str .= '<a href="javascript:;" class="prev"><img src="'.TB_IMG_URL.'/m-pager-prev.png" alt="prev"></a>'.PHP_EOL;
	}

    if($total_page > 1) {
		$str .= '<div class="pager">'.PHP_EOL;
		$str .= '		<span class="now">'.$cur_page.'</span>'.PHP_EOL;		
		$str .= '		<span>/</span>		' .PHP_EOL;		
		$str .= '		<span class="total">'.$total_page.'</span>'.PHP_EOL;					
		$str .= '</div>'.PHP_EOL;
	}

	if($total_page > $cur_page) {
		$str .= '<a href="'.$url.($cur_page+1).$add.'" class="next"><img src="'.TB_IMG_URL.'/m-pager-next-on.png" alt="next-on"></a>'.PHP_EOL;
	} else {
		$str .= '<a href="javascript:;" class="next"><img src="'.TB_IMG_URL.'/m-pager-next.png" alt="next.n"></a>'.PHP_EOL;
	}

    return "<div class=\"pager-box\">{$str}</div>";
}
...
function check_demo()
{
	if(!in_array($_SERVER['REMOTE_ADDR'], array('112.187.136.134','1.244.139.27','118.47.197.208'))) {
...
function get_sns_share_link($sns, $url, $title, $image_url)
{
    ...
    switch($sns) {
		case 'facebook':
			...
			$str = '<a href="'.$facebook_url.'" onclick="'.$str.'" target="_blank"><img src="'.$image_url.'"><p>페이스북</p></a>'.PHP_EOL;
            break;
        case 'twitter':
		...
		case 'kakaostory':
			...
			$str = '<a href="'.$kakaostory_url.'" onclick="'.$str.'" target="_blank"><img src="'.$image_url.'"><p>카카오스토리</p></a>'.PHP_EOL;
            break;
		case 'naverline':
			$naverline_url = $sns_send . '&amp;sns=naverline';
            $str = 'share_sns(\'naverline\',\'' . $naverline_url . '\'); return false;';
			$str = '<a href="'.$naverband_url.'" onclick="'.$str.'" target="_blank"><img src="'.$image_url.'"><p>라인</p></a>'.PHP_EOL;
            break;
		case 'naverband':
			$naverband_url = $sns_send . '&amp;sns=naverband';
            $str = 'share_sns(\'naverband\',\'' . $naverband_url . '\'); return false;';
			$str = '<a href="'.$naverband_url.'" onclick="'.$str.'" target="_blank"><img src="'.$image_url.'"><p>밴드</p></a>'.PHP_EOL;
            break;
		case 'pinterest':
...
function insert_search($table,$values)
{
	...
	if(strpos($_SERVER['HTTP_REFERER'], TB_URL) !== false) {
		$sql = "insert into $table ($field) VALUES ($value)";	// 실제 쿼리 생성
		return sql_query($sql);
	}
}
...
// sms 사용자정보
function get_sms_userinfo($id){
	
	$api_url	=	"http://sms.jisigin.co.kr/api/sms_user_info.php";
	$res		=	get_sock($api_url."?sms_code=$id");
...
// 문자전송 (개별전송)
function sms_member_send($recv_number, $sms_content, $sms_send_type, $fld, $index_code){
	...
	if($sm['cf_sms_use'] && $recv_number) {
		...
		if($sm['cf_icode_id']) {
			...
			$userinfo = get_sms_userinfo($sm['cf_icode_id']);

			if($userinfo->e_code == "0000"){
				
				if($userinfo->sms_call_cnt > 0){
					$is_sms_send = true;
				}
			}
		}
		...
		if($is_sms_send){
			...
			if($sms_send_type == "00"){
				...
				$sms_content			=	urlencode(addslashes($sms_content));			

				$api_url	=	"http://sms.jisigin.co.kr/api/sms_send.php";
				$res		=	get_sock($api_url."?sms_code=".$sm['cf_icode_id']."&send_number=".$send_number."&recv_number=".$recv_number."&sms_content=".$sms_content);

			//회원가입
			}else if($sms_send_type == "01"){

					$mb	=	get_member($index_code);
			...
			}else if($sms_send_type == "02"){
								
				$od		=	 get_order($index_code); // 주문정보
				$pt		=	get_member($od['pt_id'], 'cellphone');
				$sr		=	get_seller_cd($od['seller_id'], 'info_tel');


				$admin_hp		=	$super['cellphone'];		
				$seller_hp		=	$sr['info_tel'];
				...
				if($sm["cf_mb_use{$fld}"] || $sm["cf_ad_use{$fld}"] || $sm["cf_re_use{$fld}"] || $sm["cf_sr_use{$fld}"]){
					...
					$recv_numbers = array($recv_number, $admin_hp, $partner_hp, $seller_hp);
					$send_number = conv_number($sm['cf_sms_recall']);
					...
					for($s=0; $s<count($sms_send_use); $s++) {
						...
						$sms_content = rpc($sms_content, "{주문번호}", $index_code);
...
// 문자전송 (회원가입)
function icode_sms_send($mb_id, $fld)
{
	global $config, $super;
...
function member_delete($mb_id)
{
	...
	// 카테고리 폴더 전체 삭제
	if($mb_id) {
		rm_rf(TB_DATA_PATH.'/category/'.$mb_id);
	}
	...
	sql_query("delete from shop_keyword where pt_id='$mb_id'"); // 키워드
...
// 검색어 인서트
function get_sql_search($word, $pt_id)
{
	if(!file_exists(TB_PATH."/DEMO")) {
		$word = trim(strip_tags($word));
		$pp_date = date('W');

		if(!$word) alert('검색키워드가 값이 넘어오지 않았습니다.');

		if(substr_count($word, "&#") > 50) {
			alert("내용에 올바르지 않은 코드가 다수 포함되어 있습니다.");
		}

		unset($value);
		$value['keyword'] = $word;
		$value['pp_date'] = $pp_date;
		$value['pt_id']   = $pt_id;

		// 키워드값이 없으면 새로 인서트한다.
		$row = sql_fetch("select * from shop_keyword where keyword='$word'");
		if(!$row['index_no']) {
			$value['scount'] = 1;
			insert_search("shop_keyword",$value);
		} else {
			if($row['pp_date'] < $pp_date) {
				// 이번주보다 이전 검색된 키워드면 지난주 검색 카운터를 업데이트하고 새로운 카운터 1을 증가
				$value['old_scount'] = $row['scount'];
				$value['scount'] = 1;
			} else if($row['pp_date'] == $pp_date) {
...
// 상품 이미지를 얻는다
function get_it_goods_image($gs_id, $it_img, $wpx, $hpx=0, $img_id=''){
	...
	}else if(preg_match("/(youtube.com)/", $it_img)){
		$img = '<iframe width="560" height="315" src="'.$it_img.'" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe';

	}else if(preg_match("/(vimeo.com)/", $it_img)){
		$img = '<iframe width="560" height="315" src="'.$it_img.'" frameborder="0"  allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe';		
	}else{
...
// 모든쿼리 공용
function get_sql_precompose($sql_option='', $isDev= false)
{
...
function get_sale_price($gs_id)
{
	...
	if($gb['gb_sale'] > 0 && $member['id'] && !$gs['use_aff']) {
		...
		if(strlen($price) > 2 && $gb['gb_sale_unit'])
...
function save_goods_data($gs_id, $odrno, $od_id)
{
	...
	$columns = array_diff($columns, array("info_value", "info_gubun", "memo", "admin_memo", "info_color"));
	...
	// 상품정보를 주문서에 업데이트한다.
	$sql = " update shop_order set od_goods = '".addslashes(stripslashes($data))."' where index_no = '$odrno' ";
	sql_query($sql);
...
function sql_member_category($mb_id)
{
	...
	$target_table = 'shop_cate_'.$mb_id;
	$member_path = TB_DATA_PATH.'/category/'.$mb_id;
	$admin_path = TB_DATA_PATH.'/category/admin';

	if(!is_dir($member_path)) {

		// 카테고리 폴더 생성
		@mkdir($member_path, TB_DIR_PERMISSION);
		@chmod($member_path, TB_DIR_PERMISSION);
		...
		*/
		...
		while($entry = $d->read()) {
			...
			if(is_dir($admin_path.'/'.$entry)){
				...
				@mkdir($member_path.'/'.$entry, TB_DIR_PERMISSION);
				@chmod($member_path.'/'.$entry, TB_DIR_PERMISSION);
				while($entry2 = $dd->read()) {
					if($entry2 == '.' || $entry2 == '..') continue;
					@copy($admin_path.'/'.$entry.'/'.$entry2, $member_path.'/'.$entry.'/'.$entry2);
					@chmod($member_path.'/'.$entry.'/'.$entry2, TB_DIR_PERMISSION);
					$copy_file++;
				}
				$dd->close();
			}
	...
	if(!table_exists($target_table)) {
		include TB_ADMIN_PATH."/category/category_sql.php";
