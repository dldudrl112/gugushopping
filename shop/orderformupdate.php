...
if(empty($_POST) || !isset($_POST['ss_cart_id'])){
    //alert("잘못된 접근입니다.");
}
// 장바구니 상품 재고 검사
$error = "";
...
for ($i = 0; $row = sql_fetch_array($result); $i++) {
    ...
    if ($row['io_id']) {
        // 옵션의 재고 (창고재고수량)
        $optsql = " select io_stock_qty, payment_cycle
			   from shop_goods_option
			  where gs_id = '{$row['gs_id']}'
				and io_id = '{$row['io_id']}'
				and io_type = '{$row['io_type']}'
				and io_use = '1' ";
        $opt = sql_fetch($optsql);
        $it_stock_qty = (int)$opt['io_stock_qty'];
        $payment_cycle = (int)$opt['payment_cycle'];
...
for ($i = 0; $i < count($gs_id); $i++) {
    ...
    // 주문완료 후 쿠폰발행
    $gs = get_goods($gs_id[$i], 'gname, use_aff, autobill');
    ...
    //주문완료면 데이터 등록
    if ($insert_id && $dan == 2) {
        $memo = "'" . $gs["gname"] . "' 결제가 정상적으로 처리되었습니다.";
        boardRegData('43', $member["id"], $memo);
    }
...
if (in_array($_POST['paymethod'], array('무통장', '포인트'))) {
    ...
    // 주문완료 문자전송
    sms_member_send($od['cellphone'], '', '02', 2, $od_id);
...
if (in_array($_POST['paymethod'], array('무통장', '포인트'))) {
    ...
    goto_url(TB_SHOP_URL . "/billing.php?pg={$_POST['paymethod']}&od_id={$od_id}");
