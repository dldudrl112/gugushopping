...
if ($gs['index_no']) {
    ...
    $opt1_subject = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt1_subject'])));
    $opt2_subject = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt2_subject'])));
    $opt3_subject = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt3_subject'])));

    $opt1_val = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt1'])));
    $opt2_val = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt2'])));
    $opt3_val = preg_replace(TB_OPTION_ID_FILTER, '', trim(stripslashes($_POST['opt3'])));
...
if ($po_run) {
    ...
    <table class="mart20">
        ...
        <tr>
            ...
            <th scope="col" class="tac">결제주기</th>
        ...
        if ($gs['index_no']) {
            for ($i = 0; $row = sql_fetch_array($result); $i++) {
                ...
                $opt_use = $row['io_use'];
                $opt_payment_cycle = (int)$row['payment_cycle'];
                $tmp_opt = "";
                $tmp_opt .= $opt_1;
                if ($opt_2)
                    $tmp_opt .= ' &gt; ' . $opt_2;
                ...
                <tr>
                    ...
                    <td class="tac">
                        <select name="opt_payment_cycle[]" class="frm_input">
                            <option value="0">-- 결제주기 --</option>
                            <?php
                            for ($j = 1; $j <= 12; $j++) {
                                $month_selected = $opt_payment_cycle === $j ? 'selected' : '';
                                echo "<option value='{$j}' {$month_selected}>{$j}개월 마다</option>";
                            }
                            ?>
                        </select>
                    </td>
                    ...
                    <td class="tac">
                        <input type="text" name="opt_stock_qty[]" value="<?php echo $opt_stock_qty; ?>"
                               class="frm_input wfull">
                    </td>
                    <td class="tac">
                        <input type="text" name="opt_noti_qty[]" value="<?php echo $opt_noti_qty; ?>"
                               class="frm_input wfull">
                    </td>
                    <td class="tac">
                        <select name="opt_use[]">
                        ...
                        <tr>
                            ...
                            <td class="tac">
                                <select name="opt_payment_cycle[]" class="frm_input">
                                    ...
                                    for ($j = 1; $j <= 12; $j++) {
                                        $month_selected = $opt_payment_cycle === $j ? 'selected' : '';
                                        echo "<option value='{$j}' {$opt_payment_cycle}>{$j}개월 마다</option>";
                                    }
                                    ?>
                                </select>
                            </td>
                            <td class="tac">
                                <input type="text" name="opt_price[]" value="<?php echo $opt_price; ?>"
                                       class="frm_input wfull">
                            </td>
                            <td class="tac">
                                <input type="text" name="opt_stock_qty[]" value="<?php echo $opt_stock_qty; ?>"
