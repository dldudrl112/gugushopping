...
// 회원인 경우
if($is_member)
    $sql_common = " from shop_order where mb_id = '{$member['id']}' and subscription_id >= 1";
else if($od_id && $od_pwd) // 비회원인 경우 주문서번호와 비밀번호가 넘어왔다면
    $sql_common = " from shop_order where od_id = '$od_id' and od_pwd = '$od_pwd' and subscription_id >= 1";
else // 그렇지 않다면 로그인으로 가기
    goto_url(TB_BBS_URL.'/login.php?url='.$urlencode);


$sql = " select count(DISTINCT subscription_id) as cnt " . $sql_common;
$row = sql_fetch($sql);
$total_count = $row['cnt'];
...
$sql_order = " group by subscription_id order by index_no desc ";
...
$order= [];
if(!empty($od)) {
    $gs= sql_fetch("select * from shop_goods where index_no= '{$od['gs_id']}'");
    include_once(TB_PATH."/classes/Subscription.php");
    $subscription= Subscription::getInstance()->show($od['subscription_id']);
    $order= Subscription::getInstance()->decodeOrderData();
}
$action_url	=	TB_HTTPS_SHOP_URL.'/my_shipping_update.php';

include_once(TB_THEME_PATH.'/my_shipping.skin.php');
