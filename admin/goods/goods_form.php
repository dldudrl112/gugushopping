...
if ($w == "") {
	...
	if ($gs["tc_code"]) {
		$tc	=	sql_fetch("select name from shop_teacher where index_no =  '" . $gs["tc_code"] . "'");
	}
...
//sql_query("alter table shop_goods add is_adult tinyint not null default 0");성인인증 추가
...
<form name="fregform" method="post" onsubmit="return fregform_submit(this)" enctype="MULTIPART/FORM-DATA">
	...
	<input type="hidden" name="tc_code" value="<?= $gs["tc_code"] ?>">
	...
	<section id="anc_sitfrm_ini">
		...
		<th scope="row">선생님</th>
		<td>
			<input type="text" name="name" value="<?php echo $tc['name']; ?>" required itemname="선생님 코드" class="required frm_input">
			<a href="./goods_teacher.php" onclick="win_open(this,'pop_goods_teacher','550','500','no');return false" class="btn_small">선생님 선택</a>
		</td>
	</tr> -->
					<tr>
					...
					<tr>
						<th scope="row">오토빌링 설정</th>
						<td class="td_label">
							<?php echo radio_checked('autobill', $gs['autobill'], '1', '사용'); ?>
							<?php echo radio_checked('autobill', $gs['autobill'], '0', '미사용'); ?>
						</td>
					</tr>
					<tr>
                    ...
                    <tr>
                        <th scope="row">성인 인증 상품</th>
                        <td class="td_label">
                            <?php echo radio_checked('is_adult', $gs['is_adult'], '1', '사용'); ?>
                            <?php echo radio_checked('is_adult', $gs['is_adult'], '0', '미사용'); ?>
                        </td>
                    </tr>
                ...
                <?php
                    ...
                    <tr class="item_img_fld">
                        <th scope="row">성인 썸네일 노출</th>
                        <td>
                            <?php echo radio_checked('is_adult_img', $gs['is_adult_img'], '1', '사용'); ?>
                            <?php echo radio_checked('is_adult_img', $gs['is_adult_img'], '0', '미사용'); ?>

                            <span class="fc_197">(400 * 400)</span>
                            <span class="">
                                <input type="file" name="new_adult_image" value="<?php echo $gs['adult_img'];?>">
                                <input type="hidden" name="adult_image">
                                <?php
                                //sql_query("alter table shop_goods add is_adult_img tinyint not null default 0, add adult_img varchar(255) not null default ''");
                                $adultImage= TB_DATA_URL."/goods/".$gs['adult_img'];
                                if($gs['is_adult_img'] && file_exists(TB_DATA_PATH . "/goods/".$gs['adult_img'])){
                                    echo "<a href='{$adultImage}' target='_blank' class='btn_small bx-white marr7'>미리보기</a> <label class='marr7'><input type='checkbox' name='deleteAdultImage' value='{$adultImage}'>삭제</label>";
                                }
                                ?>
					...
					<tr>
						<th scope="row" style="color:coral;">개인정보 동의</th>
						<td>
							<?php echo editor_html('memo2', get_text(stripcslashes($gs['memo2']), 0)); ?>
						</td>
					</tr>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<div id="sit_option_frm">
								<?php include_once(TB_ADMIN_PATH . '/goods/goods_option.php'); ?>
							</div>
					...
					<tr>
						...
						<td>
							...
							<div id="sit_option_addfrm">
								<?php include_once(TB_ADMIN_PATH . '/goods/goods_spl.php'); ?>
							</div>
					...
					<tr>
						...
						<td>
							<div class="local_desc03">
								...
								?>
									<div class="dib padl10 padr10">
										<label><input type="checkbox" name="info_color[]" value="<?php echo $row['gd_color']; ?>" <?php echo $checked; ?>>
							...
							<p class="mart5 tac"><a href="<?php echo TB_ADMIN_URL; ?>/goods/goods_color.php" onclick="win_open(this,'pop_color','500','640','yes');return false;" class="btn_lsmall red itemicon">상품색상관리</a></p>
						</td>
					</tr>
				</tbody>
			</table>
	...
	<section id="anc_sitfrm_compact">
		...
		<div id="sit_compact_fields" class="tbl_frm02 mart7">
			<?php include_once(TB_ADMIN_PATH . '/goods/goods_info.php'); ?>
		</div>
	</section>
