...
$cb_encode_path = TB_PLUGIN_PATH."/chekplus/CPClient"; // NICE로부터 받은 암호화 프로그램의 위치
if(PHP_INT_MAX !== 2147483647) // 64-bit
    $cb_encode_path = TB_PLUGIN_PATH."/chekplus/CPClient_64bit"; // NICE로부터 받은 암호화 프로그램의 위치
...
//if(is_mobile())
//    $opener_href = TB_MBBS_URL.'/register_form.php';
//else
    $opener_href = TB_BBS_URL.'/register_form.php';
if(!empty($sReserved2) && strpos($sReserved2, TB_URL) !== false){
    $opener_href = $sReserved2;
}

if (!isset($_REQUEST["EncodeData"]) || $enc_data == "") {
    die('error');
}
$REQ_SEQ= get_session('REQ_SEQ');
//var_dump($_REQUEST);
//die();
if(empty($REQ_SEQ)){
    echo '
...
$sql = " update shop_joincheck 
            set j_ciphertime	 = '$ciphertime',
				...
				where j_key		 = '$REQ_SEQ'";
sql_query($sql);
if($is_member && $member['id']){
    $value= ['checked_birthday'=> $birthdate, 'confirmed_at'=> date("Y-m-d")];
    print_r($value);
