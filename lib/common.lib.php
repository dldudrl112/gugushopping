...
// 게시판 리스트 가져오기
function board_latest_data($boardid, $len, $rows, $pt_id, $where = ""){
	...
	$sql = "select * from shop_board_{$boardid} $sql_where $where order by wdate desc $rows_sql ";
	$res = sql_query($sql);
	for($i=0;$row=sql_fetch_array($res);$i++){
		$list[$i]['subject']		=	cut_str($row['subject'],$len);
		$list[$i]['wdate']		=	date('Y-m-d',intval($row['wdate'],10));
	}
...
// 상품 가격정보의 배열을 리턴
function get_price($gs_id, $msg='원')
{
...
function get_buy_button_unit($msg, $gs_id, $sw_direct){
	global $gs, $pt_id;
	...
	if($sw_direct == "buy"){
		...
		$sw_css	=	"";
		$sw_img	=	"<img src=\"".TB_IMG_URL."/call_dibs_ico.png\" alt=\"ico2\">";	
	}

	if($msg) {
		$str .= "<a href=\"javascript:alert('$msg');\" class=\"".$sw_css."\">".$sw_img."</a></span>";
	} else {
		if($sw_direct == "wish") {
			$str .= "<a href=\"javascript:item_wish(document.fbuyform);\" class=\"".$sw_css."\">".$sw_img."</a>";
		} else {
			$str .= "<a href=\"javascript:fbuyform_submit('".$sw_direct."');\" class=\"".$sw_css."\">".$sw_img."</a>";
		}
	}
	...
	if($subj_count > 1) {
		...
		for($i=0; $i<$subj_count; $i++) {
			...
			if($opt_count) {
				...
				$str .= '<span class="option-con">'.PHP_EOL;
				$str .= '<label for="it_option_'.$seq.'" class="vi_txt_li">'.$subj[$i].'</label>'.PHP_EOL;
		...
		$str .= '<span class="option-con">'.PHP_EOL;
		$str .= '<label for="it_option_1" class="vi_txt_li">'.$subj[0].'</label>'.PHP_EOL;
...
// 상품 선택옵션 - 옵션 변경 
function get_item_options_gugu($gs_id, $subject,$io_id,$od_id)
{
	if(!$gs_id || !$subject)
		return '';
	...
	$sql		=	" select A.*,  B.ct_option, B.ct_qty  from shop_goods_option A
					  left outer join shop_cart B ON A.io_id = B.io_id AND B.od_id = '".$od_id."'
					  where A.gs_id = '".$gs_id."' 
					  order by A.io_no asc ";
	...
	if($subj_count > 1) {
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			if($row["ct_option"] != ""){
				if(strpos($row["ct_option"], "주기") !== false){

					$arr_ct_option		=	explode("/", $row["ct_option"]);						

					if(count($arr_ct_option) > 1){
						
						for($j=0;$j<count($arr_ct_option);$j++){				
							$arr_cycle_text	=	explode(":",trim($arr_ct_option[$j]));

							if($arr_cycle_text[0] == "주기"){
								$select_subj	=	trim($arr_ct_option[$j]);
							}

							$select_io_id[$j]				=	$arr_cycle_text[1];
							$select_io_type[$j]			=	$row['io_type'];
							$select_io_price[$j]			=	$row['io_price'];
						}
					}
				}									
			}
		...
		for($i=0; $i<$subj_count; $i++) {
			...
			if($opt_count) {
				...
				if($subj[$i] == "주기"){
					$str .= '<div class="content-box" style="display:none;">'.PHP_EOL;
					$str .= '		<input type="hidden" name="select_subj" value="'.$select_subj.'">'.PHP_EOL;
				}else{
					$str .= '<div class="content-box">'.PHP_EOL;				
				}
				...
				for($k=0; $k<$opt_count; $k++) {
					...
					if($opt_val) {
						...
						$str .= '<label for="it_option_'.$seq.'">'.PHP_EOL;

						if(trim($select_io_id[$k]) == trim($opt_val)){
							$str .= '<input type="radio" name="size" value="'.$opt_val.','.$select_io_price[$i].','.$select_io_type[$i].','.$amt.','.$subj[$i].":".$opt_val.'" id="option-size-'.$seq.'" checked>'.PHP_EOL;						
						}else{
							$str .= '<input type="radio" name="size" value="'.$opt_val.','.$select_io_price[$i].','.$select_io_type[$i].','.$amt.','.$subj[$i].":".$opt_val.'" id="option-size-'.$seq.'">'.PHP_EOL;						
						}

						$str .= '<span>'.$opt_val.'</span>'.PHP_EOL;
						$str .= '</label>'.PHP_EOL;		
					}
				}
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			$str .= '<label for="it_option_'.$row['io_id'].'">'.PHP_EOL;

			if($io_id ==$row['io_id'] ){
				$str .= '<input type="radio" name="size" value="'.$row['io_id'].','.$row['io_price'].','.$row['io_type'].','.$amt.','.$subj[0].":".$row['io_id'].'" id="option-size-'.$row['io_id'].'" checked>'.PHP_EOL;
			}else{
				$str .= '<input type="radio" name="size" value="'.$row['io_id'].','.$row['io_price'].','.$row['io_type'].','.$amt.','.$subj[0].":".$row['io_id'].'" id="option-size-'.$row['io_id'].'">'.PHP_EOL;
			}
			$str .= '<span>'.$row['io_id'].'</span>'.PHP_EOL;
			$str .= '</label>'.PHP_EOL;		
		}
...
// 상품 추가옵션 주기 가져오기
function get_item_supply_gugu($gs_id, $subject)
{
	if(!$gs_id || !$subject)
		return '';

	$sql = " select * from shop_goods_option where io_type = '1' and gs_id = '$gs_id' and io_use = '1' order by io_no asc limit 1";
	$result = sql_query($sql);
	if(!sql_num_rows($result))
		return '';
...
function get_item_supply($gs_id, $subject)
{
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($opt_id[1]) {
			...
			*/
			...
			$options[$opt_id[0]][] = '<option value="'.$opt_id[1].','.$row['io_price'].','.$io_stock_qty.',0">'.$opt_id[1]."개월".$price.$soldout.'</option>';
		}
	}
	...
	for($i=0; $i<$subj_count; $i++) {
		...
		if($opt_count) {
			...
			$select .= '<option value="">'.$subj[$i].' 선택안함</option>'.PHP_EOL;
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if(!$xls) {
			...
			if($row['io_type'])
				...
				//$str .= $row['ct_option']." ".display_qty($row['ct_qty'])." (".$price_plus.display_price($row['io_price']).")".PHP_EOL;
				$str .= $row['ct_option'];
			else
...
// 상담 구분 스티커
function counseling_sticker($counseling_type,$device){
	...
	if($device == "pc"){
		...
		if($counseling_type == 1){
			$str = "<div class=\"state state1\">상담가능<img src=\"".TB_IMG_URL."/content_tel.png\" alt=\"tel\"></div>";	
		}else if($counseling_type == 2){
			$str = "<div class=\"state state2\">상담중<img src=\"".TB_IMG_URL."/content_tel1.png\" alt=\"tel\"></div>";		
		}else if($counseling_type == 3){
			$str = "<div class=\"state state3\">상담종료<img src=\"".TB_IMG_URL."/content_tel2.png\" alt=\"tel\"></div>";		
		}else{
			$str = "<div class=\"state state1\">상담가능<img src=\"".TB_IMG_URL."/content_tel.png\" alt=\"tel\"></div>";		
		}
	}
	...
	if($device == "mo"){
		...
		if($counseling_type == 1){
			$str = "<div class=\"state state1\">상담가능<img src=\"".TB_IMG_URL."/m_content_tel.png\" alt=\"tel\"></div>";	
		}else if($counseling_type == 2){
			$str = "<div class=\"state state2\">상담중<img src=\"".TB_IMG_URL."/m_content_tel1.png\" alt=\"tel\"></div>";		
		}else if($counseling_type == 3){
			$str = "<div class=\"state state3\">상담종료<img src=\"".TB_IMG_URL."/m_content_tel2.png\" alt=\"tel\"></div>";		
		}else{
			$str = "<div class=\"state state1\">상담가능<img src=\"".TB_IMG_URL."/m_content_tel.png\" alt=\"tel\"></div>";		
		}
	}
...
// 알림글 데이터 등록
function boardRegData($boardid, $mb_id, $content){
	
	if($boardid && $mb_id && $content){

		$fid	=	get_next_num("shop_board_".$boardid);
		$mb	=	get_member($mb_id, 'index_no, name, email, cellphone');
