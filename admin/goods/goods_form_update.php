...
$value['tc_code']			= $_POST['tc_code']; //선생님 코드
$value['gname']				= $_POST['gname']; //상품명
$value['isopen']				= $_POST['isopen']; //진열상태
$value['isopen_test']				= $_POST['isopen_test']; //진열상태
$value['explan']				= addslashes($_POST['explan']); //짧은설명
...
$value['autobill']			= $_POST['autobill']; //과세구분
...
$value['is_adult']		= (int)$_POST['is_adult']; //성인인증 유무
$value['is_adult_img']		= (int)$_POST['is_adult_img']; //성인 썸네일 사용 유무
$value['adult_img']		= $_POST['new_adult_img']; //성인 썸네일

if(isset($_POST['deleteAdultImage']) && empty($_POST['deleteAdultImage'])) {
    $upl->del($_POST['deleteAdultImage']);
    $value['adult_img'] = '';
}
if(isset($_FILES['new_adult_image']) && !empty($_FILES['new_adult_image']) && $_FILES['new_adult_image']['name']) {
    if(!preg_match("/\.(gif|jpg|png)$/i", $_FILES['new_adult_image']['name'])) {
        alert("이미지가 gif, jpg, png 파일이 아닙니다.");
    }
    $value['adult_img'] = $upl->upload($_FILES['new_adult_image']);
}
...
if($option_count) {
    ...
    $sql = " insert into shop_goods_option
                    ( `io_id`, `io_type`, `gs_id`, `io_price`, `io_stock_qty`, `io_noti_qty`, `io_use`, `payment_cycle` )
                VALUES ";
    for($i=0; $i<$option_count; $i++) {
        $optPaymentCycle= (int)$_POST['opt_payment_cycle'][$i];
        $sql .= $comma . " ( '{$_POST['opt_id'][$i]}', '0', '$gs_id', '{$_POST['opt_price'][$i]}', '{$_POST['opt_stock_qty'][$i]}', '{$_POST['opt_noti_qty'][$i]}', '{$_POST['opt_use'][$i]}', {$optPaymentCycle} )";
        $comma = ' , ';
    }
    sql_query($sql);
}
