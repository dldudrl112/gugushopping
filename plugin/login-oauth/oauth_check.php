...
// 소셜아이디(sns_id) 체크
$mb = sql_fetch(" select * from shop_member where sns_id = '{$mb_sns_id}' ", false);
if(!isset($mb['sns_id'])) {
	// sn_id 필드생성
...
if($mb['id']) { // 가입된 회원이면
	...
	if(!$mb['sns_id']) {
		$mb['sns_id'] = $mb_sns_id;
		sql_query(" update shop_member set sns_id = '{$mb_sns_id}' where id = '{$mb['id']}' ", false);
	}
	...
	unset($value);
	$value['id']					=	$sns_id;						//회원아이디
	$value['sns_id']				=	$mb_sns_id;				//sns_id
	$value['name']				=	$mb_name;				//회원명
	$value['nickname']			=	$mb_nick;					//닉네임
	$value['passwd']			=	$tmp_pw;					//비밀번호
	$value['email']				=	$mb_email;				//이메일
	$value['cellphone']			=	$mb_phone;				//연락처
	$value['birth_year']			=	$mb_birth_year;			//생년
	$value['birth_month']		=	$mb_birth_month;		//생월
	$value['birth_day']			=	$mb_birth_day;			//생일
	$value['birth_type']			=	$mb_birth_type;			//양력/음력
	$value['gender']				=	$mb_gender;				//성별
	//$value['mem_img']		=	$mb_img;					//회원 이미지
	//$value['mem_img_type']	=	$mb_img_type;			//회원 이미지
